...
def _evaluate_stable(env, agent):
    total_reward = 0.0
    obs = env.reset()
    total = 0 # total request
    accept = 0 # total accept
    ...
    while True:
        ...
        choosed_action, _ = agent.predict(obs)
        choosed_action = int(choosed_action)
...
def evaluate_stable(env_list_path, agent, render=False):
    ...
    for env in env_list:
        total_reward, total, accept, \
        total_sd, accept_sd, total_ed, \
        accept_ed, total_es, accept_es = _evaluate_stable(env, agent)
        eval_reward_list.append(total_reward)
        total_req_list.append(total)
        acc_req_list.append(accept)
        total_sd_list.append(total_sd)
