...
class Agent():
    ...
    def learn(self, batch_obs, batch_action, batch_reward, batch_next_obs, batch_terminal):
        ...
        with torch.no_grad(): 
            ...
            target_value = batch_reward + (1 - batch_terminal.view(-1,1))*target_q
            target_value = (target_value*self.gamma-pred_value)*self.alpha + pred_value
...
def train(show_baseline=False, continue_train=False, \
    ...
    memory_warmup_size = 2000, batch_size = 32, learning_rate = 0.0001, \
    gamma = 0.9, alpha = 0.9):
